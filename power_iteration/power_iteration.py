...
def power_iteration_np(A, num_simulations=10, init_vect=None, approximate_scale=1.00):
	...
	return np.sqrt(b_k.T @ A @ b_k / (b_k.T @ b_k  )) * approximate_scale  # add 10 percent so we always shoot a bit larger 
# --- END NUMPY CODE ---


# --- TF CODE --- 
def power_iteration_tf(A, sess, num_simulations=10, init_vect=None, approximate_scale=1.00):
	A = tf.transpose(A) @ A
	...
	return np.sqrt(sess.run(tf.transpose(b_k) @ A @ b_k / (tf.transpose(b_k) @ b_k))) * approximate_scale  # add 10 percent so we always shoot a bit larger 
# --- END TF CODE --- 

# --- RENAME FASTEST (see test_speed.py for experiment) --- 
def power_iteration(A, num_simulations=10, init_vect=None, approximate_scale=1.00): 
	return power_iteration_np(A, num_simulations, init_vect, approximate_scale)
